/**
 * Fill in missing phone numbers for all outbound leads.
 * Uses Apollo people/match to get org phone (main company line) — no extra credit cost.
 *
 * Usage:
 *   APOLLO_API_KEY=... QUEUE_AUTH=... API_BASE=https://ghl-pillar.vercel.app \
 *   node scripts/repair-phones.mjs
 */

const APOLLO_API_KEY = process.env.APOLLO_API_KEY;
const QUEUE_AUTH = process.env.QUEUE_AUTH;
const API_BASE = process.env.API_BASE || 'https://ghl-pillar.vercel.app';
const CONCURRENCY = 8;
const BATCH_PAUSE_MS = 600;

if (!APOLLO_API_KEY) { console.error('APOLLO_API_KEY missing'); process.exit(1); }
if (!QUEUE_AUTH) { console.error('QUEUE_AUTH missing'); process.exit(1); }

async function queueApi(payload) {
  const res = await fetch(`${API_BASE}/api/apollo-sales-queue`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-queue-auth': QUEUE_AUTH },
    body: JSON.stringify(payload),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data.success) throw new Error(`Queue API ${payload.action}: HTTP ${res.status} – ${JSON.stringify(data).slice(0, 200)}`);
  return data;
}

async function apolloPhone(apolloId) {
  const res = await fetch('https://api.apollo.io/api/v1/people/match', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Api-Key': APOLLO_API_KEY },
    body: JSON.stringify({ id: apolloId, reveal_personal_emails: false }),
  });
  if (res.status === 429) throw new Error('RATE_LIMIT');
  if (!res.ok) return null;
  const data = await res.json().catch(() => null);
  const p = data?.person;
  if (!p) return null;
  return (
    p.sanitized_phone || p.direct_dial_phone ||
    p.phone_numbers?.find((n) => n.type === 'work_direct')?.raw_number ||
    p.phone_numbers?.[0]?.raw_number ||
    (typeof p.phone_numbers?.[0] === 'string' ? p.phone_numbers[0] : null) ||
    p.organization?.phone ||
    p.organization?.primary_phone?.number ||
    null
  );
}

async function sleep(ms) { return new Promise((r) => setTimeout(r, ms)); }

async function run() {
  // Fetch all leads without a phone that have an apollo_id.
  const data = await fetch(`${API_BASE}/api/apollo-sales-queue?source=outbound`, {
    headers: { 'x-queue-auth': QUEUE_AUTH },
  }).then((r) => r.json());

  const needPhone = (data.contacts || []).filter((c) => !c.phone && c.apolloId);
  console.log(`${needPhone.length} leads need a phone number`);
  if (!needPhone.length) { console.log('Nothing to do.'); return; }

  let patched = 0;
  let noPhone = 0;
  const totalBatches = Math.ceil(needPhone.length / CONCURRENCY);

  for (let b = 0; b < totalBatches; b++) {
    const batch = needPhone.slice(b * CONCURRENCY, (b + 1) * CONCURRENCY);

    const results = await Promise.all(batch.map(async (lead) => {
      try {
        const phone = await apolloPhone(lead.apolloId);
        return phone ? { id: lead.id, phone } : null;
      } catch (err) {
        if (err.message === 'RATE_LIMIT') {
          process.stderr.write('Rate limited — pausing 10s\n');
          await sleep(10000);
          try {
            const phone = await apolloPhone(lead.apolloId);
            return phone ? { id: lead.id, phone } : null;
          } catch { return null; }
        }
        return null;
      }
    }));

    const updates = results.filter(Boolean);
    noPhone += batch.length - updates.length;

    process.stderr.write(`Batch ${b + 1}/${totalBatches}: ${updates.length}/${batch.length} with phone\n`);

    if (updates.length) {
      const r = await queueApi({ action: 'patch-phones', updates });
      patched += r.patched || 0;
    }

    if (b < totalBatches - 1) await sleep(BATCH_PAUSE_MS);
  }

  console.log(JSON.stringify({ patched, noPhone }));
}

run().catch((e) => { console.error(e); process.exit(1); });

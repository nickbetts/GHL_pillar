...
function signature(sender) { return safeText(sender.sender_signature).replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]*>/g, '').trim() || ['Best,', sender.name || sender.email, sender.sender_title || '', sender.sender_email || sender.email].filter(Boolean).join('\n'); }
...
function nextBusinessTime(now, hour) { const result = new Date(now); result.setMinutes(0, 0, 0); if (result.getHours() < hour) { result.setHours(hour); return result; } result.setDate(result.getDate() + 1); result.setHours(hour); while ([0, 6].includes(result.getDay())) result.setDate(result.getDate() + 1); return result; }
  ...
  try {
    ...
    const due = await sql`
      SELECT e.id AS enrollment_id, e.current_step, e.next_step_due, e.lead_id, c.id AS campaign_id, c.status AS campaign_status,
             ...
             l.first_name, l.name AS lead_name, l.company_name, l.email, l.owner_id, l.archived_at,
    ...
    for (const item of due) {
      ...
      const senderEmail = safeText(item.sender_email || item.sender_account_email).toLowerCase();
      if (!validEmail(senderEmail)) { results.push({ enrollmentId:item.enrollment_id, status:'failed', error:'Sender is not configured' }); continue; }
      ...
      try {
        ...
        const sendRows = await sql`INSERT INTO email_campaign_sends (enrollment_id, step_id, sender_user_id, email_send_log_id, provider_message_id, status, rendered_subject, rendered_body, sent_at) VALUES (${item.enrollment_id}, ${item.step_id}, ${item.sender_user_id}, ${logs[0].id}, ${response.id || null}, 'sent', ${subject}, ${textBody}, now()) RETURNING id`;
        ...
        else await sql`UPDATE email_campaign_enrollments SET current_step = ${item.step_order}, next_step_due = ${nextBusinessTime(new Date(Date.now() + nextStep[0].wait_days * 86400000), nextStep[0].send_hour).toISOString()}::timestamptz, last_sent_at = now(), updated_at = now() WHERE id = ${item.enrollment_id}`;
        results.push({ enrollmentId:item.enrollment_id, status:'sent', sendId:sendRows[0].id });
      } catch (error) { await sql`UPDATE email_campaign_enrollments SET next_step_due = now() + interval '30 minutes', updated_at = now() WHERE id = ${item.enrollment_id}`; results.push({ enrollmentId:item.enrollment_id, status:'failed', error:error.message }); }
    }

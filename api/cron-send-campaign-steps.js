...
function bookingUrl(lead, campaignUrl) { const url = new URL(campaignUrl || 'https://click.i3media.net/growth'); if (lead.first_name) url.searchParams.set('firstName', lead.first_name); if (lead.company_name) url.searchParams.set('companyName', lead.company_name); return url.toString(); }
function buildUnsubscribeUrl(email) { const secret = process.env.UNSUBSCRIBE_SECRET || process.env.QUEUE_PASSWORD || ''; const sig = crypto.createHmac('sha256', secret).update(String(email).trim().toLowerCase()).digest('hex'); const base = (process.env.APP_URL || 'https://ghl-pillar.vercel.app').replace(/\/$/, ''); return `${base}/unsubscribe?email=${encodeURIComponent(email)}&sig=${sig}`; }
...
async function sendMailgun({ from, to, subject, text, html, replyTo, unsubscribeUrl }) { const key = process.env.MAILGUN_API_KEY; const domain = process.env.MAILGUN_DOMAIN; if (!key || !domain) throw new Error('Mailgun is not configured'); const payload = new URLSearchParams({ from, to, subject, text, html, 'h:Reply-To': replyTo }); if (unsubscribeUrl) { payload.append('h:List-Unsubscribe', `<${unsubscribeUrl}>`); payload.append('h:List-Unsubscribe-Post', 'List-Unsubscribe=One-Click'); } const response = await fetch(`${(process.env.MAILGUN_BASE_URL || 'https://api.mailgun.net').replace(/\/$/, '')}/v3/${domain}/messages`, { method:'POST', headers:{ Authorization:`Basic ${Buffer.from(`api:${key}`).toString('base64')}`, 'Content-Type':'application/x-www-form-urlencoded' }, body:payload.toString() }); const data = (response.headers.get('content-type') || '').includes('application/json') ? await response.json().catch(() => ({})) : { message: await response.text() }; if (!response.ok) throw new Error(data.message || `Mailgun request failed (${response.status})`); return data; }

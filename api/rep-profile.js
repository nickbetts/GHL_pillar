 ...
 *   POST /api/rep-profile { action: 'update-background', workspaceBackground }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'me') {
      const rows = await sql`
        SELECT email, name, ghl_owner_id, avatar, avatar_color, zen_background, sender_email
    ...
    if (action === 'update-background') {
      const background = normalizeWorkspaceBackground(body.workspaceBackground);
      if (!background.ok) return res.status(400).json({ success: false, error: background.error });
      const rows = await sql`
        UPDATE app_users
        SET zen_background = ${background.value}, updated_at = now()
        WHERE lower(email) = ${identity.email.toLowerCase()}

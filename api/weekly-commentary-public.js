...
function getCache() {
  if (!globalThis.__weeklyCommentaryPublicCache) globalThis.__weeklyCommentaryPublicCache = new Map();
  return globalThis.__weeklyCommentaryPublicCache;
}
...
export default async function handler(req, res) {
  ...
  if (!isAuthorized(req)) {
    return res.status(401).json({ success: false, error: 'Invalid share token' });
  }
  ...
  if (cached) {
    return res.status(200).json({ success: true, available: true, releaseAt: release.releaseAt.toISOString(), commentary: cached.commentary, generatedAt: cached.generatedAt, source: cached.source, isPublic: true });
  }
  ...
  return res.status(200).json({ success: true, available: true, releaseAt: release.releaseAt.toISOString(), ...payload, isPublic: true });
}
